      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-button type="warning" @click="modifyStaff(scope.row.id)"
              >修改</el-button
            >
            <el-button type="danger" @click="deleteStaff(scope.row.id)"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    modifyStaff(val) {
      this.$router.push({
        path: "/dept/detail",
        query: { id: val, department: 214 }
      }); //用go刷新
    },
    //添加员工
    creatStaff() {
      this.$router.push({ path: "/dept/add", query: { department: 214 } }); //用go刷新
    }
  }
};
</script>
